-- Migration 0005: drop LLM/unused tables, clear data, reseed correctly
-- =====================================================

-- Drop unused tables (LLM/old architecture)
DROP TABLE IF EXISTS tariff_eval_cache           CASCADE;
DROP TABLE IF EXISTS sector_universal_questions  CASCADE;
DROP TABLE IF EXISTS sector_scoring_rules        CASCADE;
DROP TABLE IF EXISTS sector_product_recommendations CASCADE;
DROP TABLE IF EXISTS product_tariffs             CASCADE;

-- =====================================================
-- Clear all seed data (FK-safe order)
-- =====================================================
...
INSERT INTO insurers (slug, name) VALUES ('amtrust', 'AmTrust Europe');
...
(VALUES
  ...
  ('amtrust-colpa-grave-extra',
   ...
   'RC Colpa Grave per medici dipendenti SSN e strutture private. Tutela rivalsa Legge Gelli-Bianco.'),
...
SELECT p.id, q.key, q.label, q.type, q.opts::jsonb, q.pos, true, q.phase::text
FROM products p,
(VALUES
  ('q_gruppo_rischio_medico', 'Gruppo di rischio',
   'dropdown',
...
FROM products p,
(VALUES
  ('q_categoria_rischio_cg', 'Categoria di rischio',
   'dropdown',
   ...
   1, 'pricing'),
  ('q_retroattivita_rc', 'Retroattività RC',
   'dropdown',
...
JOIN (VALUES
  ('amtrust-medico-protetto',   'rc_professionale', 'RC Professionale',
   '["q_gruppo_rischio_medico","q_massimale_rc"]'),
...
-- =====================================================
-- Rate rows — Medico Protetto (9 gruppi × 2 massimali)
-- =====================================================
...
CROSS JOIN (VALUES
  ('grp1', 1000000,  480), ('grp1', 2000000,  720),
  ('grp2', 1000000,  600), ('grp2', 2000000,  900),
  ('grp3', 1000000,  900), ('grp3', 2000000, 1350),
  ('grp4', 1000000, 1200), ('grp4', 2000000, 1800),
  ('grp5', 1000000,  960), ('grp5', 2000000, 1440),
  ('grp6', 1000000, 1440), ('grp6', 2000000, 2160),
  ('grp7', 1000000, 1080), ('grp7', 2000000, 1620),
  ('grp8', 1000000, 1800), ('grp8', 2000000, 2700),
  ('grp9', 1000000, 2400), ('grp9', 2000000, 3600)
) AS r(grp, mass, premium)
WHERE cov.key = 'rc_professionale';

-- =====================================================
-- Rate rows — Colpagrave Extra (5 cat × 4 mass × 2 retro = 40)
-- =====================================================

INSERT INTO product_rate_rows (coverage_id, dimension_values, premium, manual_quote)
SELECT cov.id,
       jsonb_build_object('q_categoria_rischio_cg', r.cat,
                          'q_massimale_rc',         r.mass,
                          'q_retroattivita_rc',      r.retro),
       r.premium, false
FROM product_coverages cov
JOIN products p ON p.id = cov.product_id AND p.slug = 'amtrust-colpa-grave-extra'
CROSS JOIN (VALUES
  ('cod_01', 1000000, '10_anni',     320), ('cod_01', 1000000, 'illimitata',  420),
  ('cod_01', 2000000, '10_anni',     480), ('cod_01', 2000000, 'illimitata',  630),
  ('cod_01', 3000000, '10_anni',     640), ('cod_01', 3000000, 'illimitata',  840),
  ('cod_01', 5000000, '10_anni',     960), ('cod_01', 5000000, 'illimitata', 1260),
  ('cod_02', 1000000, '10_anni',     480), ('cod_02', 1000000, 'illimitata',  630),
  ('cod_02', 2000000, '10_anni',     720), ('cod_02', 2000000, 'illimitata',  945),
  ('cod_02', 3000000, '10_anni',     960), ('cod_02', 3000000, 'illimitata', 1260),
  ('cod_02', 5000000, '10_anni',    1440), ('cod_02', 5000000, 'illimitata', 1890),
  ('cod_03', 1000000, '10_anni',     720), ('cod_03', 1000000, 'illimitata',  945),
  ('cod_03', 2000000, '10_anni',    1080), ('cod_03', 2000000, 'illimitata', 1418),
  ('cod_03', 3000000, '10_anni',    1440), ('cod_03', 3000000, 'illimitata', 1890),
  ('cod_03', 5000000, '10_anni',    2160), ('cod_03', 5000000, 'illimitata', 2835),
  ('cod_04', 1000000, '10_anni',    1080), ('cod_04', 1000000, 'illimitata', 1418),
  ('cod_04', 2000000, '10_anni',    1620), ('cod_04', 2000000, 'illimitata', 2126),
  ('cod_04', 3000000, '10_anni',    2160), ('cod_04', 3000000, 'illimitata', 2835),
  ('cod_04', 5000000, '10_anni',    3240), ('cod_04', 5000000, 'illimitata', 4253),
  ('cod_05', 1000000, '10_anni',    1620), ('cod_05', 1000000, 'illimitata', 2126),
  ('cod_05', 2000000, '10_anni',    2430), ('cod_05', 2000000, 'illimitata', 3189),
  ('cod_05', 3000000, '10_anni',    3240), ('cod_05', 3000000, 'illimitata', 4253),
  ('cod_05', 5000000, '10_anni',    4860), ('cod_05', 5000000, 'illimitata', 6378)
) AS r(cat, mass, retro, premium)
WHERE cov.key = 'rc_colpa_grave';

-- =====================================================
-- Rate rows — Under 35 (solo massimale)
-- =====================================================
...
-- =====================================================
-- Eligibility rules
-- LOGICA: la condizione = quando è TRUE → applica l'azione (exclude / manual_quote)
-- =====================================================
...
FROM products p
JOIN (VALUES
  -- Medico Protetto: solo liberi professionisti → escludi se è dipendente
  ('amtrust-medico-protetto',
   ...
   10),
  -- Medico Protetto: più di 2 sinistri → preventivo manuale
  ('amtrust-medico-protetto',
   'Più di 2 sinistri negli ultimi 5 anni',
   '{"op":"greater_than","key":"q_sinistri_5_anni","value":2}',
   'manual_quote',
   'Più di 2 sinistri negli ultimi 5 anni: richiesto preventivo manuale',
   20),
  -- Colpagrave Extra: solo dipendenti → escludi se è libero professionista
  ('amtrust-colpa-grave-extra',
   'Solo dipendenti SSN / struttura privata',
   ...
   10),
  -- Under 35: solo liberi professionisti → escludi se è dipendente
  ('amtrust-medico-under-35',
   'Solo liberi professionisti',
   '{"op":"equals","key":"q_tipo_assicurato_medico","value":"dipendente"}',
   'exclude',
...
JOIN (VALUES
  ('Franchigia € 10.000', 0.92,
   '{"op":"equals","key":"q_franchigia_medico","value":10000}', 10),
  ('Franchigia € 25.000', 0.85,
   '{"op":"equals","key":"q_franchigia_medico","value":25000}', 11),
  ('Attività in struttura (+15%)', 1.15,
   '{"op":"equals","key":"q_attivita_struttura","value":"si"}', 20)
) AS m(name, factor, cond, prio) ON true
WHERE p.slug = 'amtrust-medico-protetto';
