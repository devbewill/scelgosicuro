...
CREATE TABLE products (
  id          BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  insurer_id  BIGINT NOT NULL REFERENCES insurers(id) ON DELETE RESTRICT,
...
CREATE TABLE product_rate_rows (
  id               BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  coverage_id      BIGINT NOT NULL REFERENCES product_coverages(id) ON DELETE CASCADE,
...
CREATE TABLE product_multipliers (
  id          BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  product_id  BIGINT NOT NULL REFERENCES products(id)          ON DELETE CASCADE,
  coverage_id BIGINT          REFERENCES product_coverages(id) ON DELETE CASCADE,
  name        TEXT NOT NULL,
...
CREATE TABLE quote_results (
  id                BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  session_id        BIGINT NOT NULL REFERENCES quote_sessions(id) ON DELETE CASCADE,
...
CREATE TABLE policies (
  id                       BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  session_id               BIGINT NOT NULL REFERENCES quote_sessions(id) ON DELETE RESTRICT,
  quote_result_id          BIGINT NOT NULL REFERENCES quote_results(id)  ON DELETE RESTRICT,
