-- Migration 0018: AmTrust Dentista Protetto
--
-- Target: odontoiatri/dentisti e ortodontisti in libera professione
-- Copertura: RC Professionale + Colpa Grave (inclusa nel premio base)
-- Dimensioni pricing: q_professione × q_massimale_rc (2M/3M/5M)
--   Le categorie tariffarie sono modellate come professioni distinte:
--     odontoiatra                          → Gruppo 1 (no implantologia)
--     odontoiatra_implantologia_osteointegrata → Gruppo 2
--     odontoiatra_implantologia_altre          → Gruppo 3
--     ortodontista                         → Gruppo 1 (stesse tariffe)
-- Retroattività illimitata: flat add-on €45 (non è dimension di rate_row)
-- Moltiplicatori: età, struttura, sinistri, franchigia
-- Add-on flat: retro illimitata (€45), ruolo apicale (€50), RCT-RCO (€100), ECM (€40)
...
FROM sectors s
CROSS JOIN (VALUES
  ('odontoiatra',                          'Odontoiatra / Dentista (senza implantologia)', 'chirurgica',     1),
  ...
  ('ortodontista',                         'Ortodontista',                                'non_chirurgica', 4)
...
-- ─────────────────────────────────────────────────────────────────────────────
-- PARTE 3 — Coverage
-- Le dimensioni di pricing sono q_professione e q_massimale_rc.
-- Non esiste alcun livello intermedio "gruppo di rischio": la professione
-- selezionata dall'utente è direttamente la chiave tariffaria.
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- PARTE 4 — Product questions (FASE C — phase=addon)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- PARTE 8 — Rate rows (12 righe: 4 professioni × 3 massimali)
--
-- odontoiatra + ortodontista → prezzi Gruppo 1 (stessa tariffa)
-- odontoiatra_implantologia_osteointegrata → prezzi Gruppo 2
-- odontoiatra_implantologia_altre          → prezzi Gruppo 3
--
-- Fonte: Tariffario AmTrust Dentista Protetto Agg. 06/2024
-- ─────────────────────────────────────────────────────────────────────────────

-- ── Gruppo 1: Odontoiatra senza implantologia + Ortodontista ─────────────────
